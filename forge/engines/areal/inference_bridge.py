"""AReaL inference bridge — routes generation to GeneratorActor via Monarch RPC.

Drop-in replacement for ``RemotevLLMEngine``. Implements AReaL's
``InferenceEngine`` interface so that ``PPOTrainer`` can use it
transparently. Also satisfies ``forge.core.protocols.InferenceBridge``.

All AReaL-specific imports (``InferenceEngine``, ``VLLMBackend``,
``WorkflowExecutor``, ``RolloutController``) are confined here.
"""
...
logger = areal_logging.getLogger("AReaLInferenceBridge")

RID_CACHE_SIZE = 128


class AReaLInferenceBridge(InferenceEngine):
    """Non-invasive bridge from AReaL's training loop to Forge's Generator.

    Holds a reference to the ``GeneratorActor`` (or ``ServiceInterface``)
    and a ``VLLMBackend`` for request/response format compatibility. All
    transport goes through Monarch RPC.
    """
    ...
    def initialize(
        ...
        self.logger = areal_logging.getLogger(
            f"[AReaLInferenceBridge Rank {engine_id}]"
        )
        ...
        self.logger.info("AReaLInferenceBridge initialised (Monarch RPC mode)")
