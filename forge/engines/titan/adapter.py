...
class TitanTrainEngine:
    ...
    def initialize(self) -> dict:
        ...
        logger.info(
            ...
            cfg.model_name, cfg.model_flavor, cfg.max_steps,
        )
        ...
        logger.info(
            ...
            rank, cfg.model_name, cfg.model_flavor,
            self._engine.dp_degree, cfg.tp,
        )
    ...
    def train_step(self, batch: dict, step: int) -> dict:
        ...
        loss_mask = self._to_tensor(batch.get("loss_mask"), device) if "loss_mask" in batch else None
        advantages = self._to_tensor(batch.get("advantages"), device) if "advantages" in batch else None
        gen_logprobs = self._to_tensor(batch.get("generator_logprobs"), device) if "generator_logprobs" in batch else None
        ...
        with engine.train_context(None):
            ...
            with engine.maybe_enable_amp:
                ...
                if self._loss_fn and advantages is not None and gen_logprobs is not None:
                    logits_shifted = logits[:, :-1, :]
                    target_shifted = target_ids[:, :-1] if target_ids.dim() > 1 else target_ids
                    loss_mask_shifted = loss_mask[:, 1:] if loss_mask is not None else None
                    gen_lp_shifted = gen_logprobs[:, 1:]
                    adv_shifted = advantages[:, 1:] if advantages.dim() > 1 else advantages
                    ...
                    loss_output = self._loss_fn(
                        ...
                        loss_mask=loss_mask_shifted if loss_mask_shifted is not None else torch.ones_like(gen_lp_shifted),
                    )
    ...
    def get_weights_spec(self) -> WeightsSpec:
        ...
        for name, param in self._engine.model_parts[0].named_parameters():
            ...
            total += param.numel()
        return WeightsSpec(
            param_names=names, param_shapes=shapes, param_dtypes=dtypes,
            total_params=total, model_path=f"{self._config.model_name}/{self._config.model_flavor}",
        )

    def state_dict_for_sync(self) -> dict:
        if not self._initialized:
            raise RuntimeError("Not initialized")
        return {k: v.cpu() for k, v in self._engine.model_parts[0].state_dict().items()}
