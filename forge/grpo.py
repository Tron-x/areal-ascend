...
async def grpo_main(config=None, run_id: int = 0, provisioner_config=None):
...
def main():
    ...
    if bare_metal_args:
        ...
        master = bare_metal_args.get("master_addr", "")
        if master:
            enable_transport(f"tcp://{master}:0")

    sys.argv = [sys.argv[0]] + remaining_argv

    context()

    asyncio.run(grpo_main(run_id=0, provisioner_config=provisioner_config))
