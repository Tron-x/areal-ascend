...
Uses Generator + TrainerActor.train_step for synchronous
rollout-and-training. All AReaL-specific logic is accessed via
the ``AReaLConfigBridge`` and ``AReaLTrainBackend`` adapters.
...
async def grpo_main(config=None, run_id: int = 0):
    ...
    Architecture::
        ...
        Generator.as_actor()     -- vLLM inference
        RewardActor.as_actor()   -- reward computation
        TrainerActor.as_actor()  -- FSDP training (via AReaLTrainBackend)
