"""AReaLForgeActor -- TorchForge-style declarative actor base class.
...
if TYPE_CHECKING:
    ...
    from areal.monarch_plugin.controller.service.interface import ServiceInterface
...
from areal.monarch_plugin.types import ProcessConfig, ServiceConfig

logger = logging.getLogger(__name__)

T = TypeVar("T", bound="AReaLForgeActor")


class AReaLForgeActor(Actor):
    ...
    Subclasses declare their resource needs as class attributes::
        ...
        class MyTrainer(AReaLForgeActor):
            procs = 4
            with_gpus = True
    ...
    async def as_service(cls: type[T], *actor_args, **actor_kwargs) -> ServiceInterface:
        ...
        """
        from areal.monarch_plugin.controller.service import Service, ServiceInterface
    ...
    @classmethod
    async def shutdown(cls, actor: AReaLForgeActor):
