...
class TrainerActor(AReaLMonarchActor):
    ...
    # procs and with_gpus are declared as class attributes in AReaLMonarchActor
    procs = 1
    with_gpus = True
    ...
    def initialize(self) -> dict:
        ...
        try:
            ...
            logger.info("[TrainerActor] Restored monkey-patches.")

        if "trainer" not in _captured:
            raise RuntimeError(
                f"Could not capture PPOTrainer instance from {script_path}. "
                "Ensure the script calls `trainer.train(workflow=...)`."
            )
    ...
    def _ensure_batch_on_device(self, batch_data: Any) -> dict:
        """Deserialise a batch if needed; move tensors to the training device."""
        import pickle
        import torch
        ...
        if isinstance(batch_data, bytes):
            try:
                ...
                logger.error(f"[TrainerActor] Failed to unpickle rollout batch: {e}")
                raise
