"""TrainerActor -- clean FSDP training actor for Monarch orchestration.

Wraps AReaL's PPOTrainer and FSDPEngine without monkey-patching.
Receives configuration via constructor, initializes training pipeline
in ``setup()``, and exposes ``train_step`` / ``push_weights`` endpoints.
"""
...
class TrainerActor(AReaLForgeActor):
    """FSDP training actor with clean engine wrapping.

    Unlike the old TrainerActor which monkey-patched PPOTrainer and FSDPEngine,
    this version:
    - Uses ``_init_rollout_override`` to inject MonarchVLLMEngine cleanly
    - Moves batch to device explicitly via ``_batch_to_device()``
    - Separates rollout and training into independent endpoints

    Lifecycle::

        actor = await TrainerActor.options(procs=4, with_gpus=True).as_actor(
            cli_args=sys.argv[1:], env_vars={...}, ...
        )
        info = await actor.initialize.call()
        result = await actor.train_on_batch.call(batch, step)
    """
    ...
    def initialize(self) -> dict:
        ...
        Sets up distributed env, loads the experiment script, captures
        the PPOTrainer instance, and returns training metadata.
        """
        if self._rank < 0:
    ...
    def _capture_trainer(self) -> tuple:
        ...
        trainer reference, and ``_init_rollout`` to inject MonarchVLLMEngine.
        Patches are restored immediately after script execution.
        """
        ...
        try:
            ...
            logger.info(f"[TrainerActor] Loading script: {script_path}")
            ...
            logger.error(f"[TrainerActor] Script execution failed: {e}")
            traceback.print_exc()
            raise RuntimeError(f"TrainerActor initialization failed: {e}") from e
        finally:
    ...
    def train_step(self, global_step: int) -> dict:
        """Combined rollout + training in one step (legacy compatibility)."""
        batch_data = self._do_rollout(global_step)
        return self._train_on_batch(batch_data, global_step)

    @endpoint
    def do_rollout(self, global_step: int) -> dict:
        """Run rollout only, return serialized batch."""
        return self._do_rollout(global_step)
    ...
    def _do_rollout(self, global_step: int) -> dict:
        import numpy as np
        import torch
    ...
    def _train_on_batch(self, batch_data: dict, global_step: int) -> dict:
        trainer = self._trainer
        epoch = global_step // self._steps_per_epoch
        step_in_epoch = global_step % self._steps_per_epoch
...
def _batch_to_device(batch_data: Any, trainer) -> dict:
    ...
    1. Pickled bytes -> unpickle then recurse
    2. Dict of lists -> convert to tensors on device
    3. Dict of tensors -> move to device if needed
    """
    import pickle
